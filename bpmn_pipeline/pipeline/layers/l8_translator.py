"""L8 — BPMN Translator: lane-aware layout + BPMN 2.0 XML serialization.
...
def validate_gate(job: Job) -> None:
    ...
    if not paths:
        raise LayerError("L8_EMPTY_OUTPUT", "No BPMN files were written.")
    for p in paths:
        if not p or not os.path.exists(p) or os.path.getsize(p) == 0:
            raise LayerError("L8_EMPTY_OUTPUT", f"BPMN output missing or empty: {p}")
    for proc in job.processes:
...
def _emit_data_objects(process_el, process_model) -> list[tuple]:
    """Data objects are no longer used — DataVar was removed with the variable-graph."""
    return []
...
def _validate_xml(xml_bytes: bytes) -> None:
    try:
        ...
        raise LayerError("L8_INVALID_XML", str(e))
